function [h_n,a_n,information] = create_room_beampattern(PlotRoom)
    ...
    % room dimensions 
    x_lim = 8; %6 + 3*rand(1);         % x position (m) - number in the range [6,9](m)
    y_lim = 8; %6 + 3*rand(1);         % y position (m) - number in the range [6,9](m)
    ...
    %idx_angle = randi(numel(Vangle));       % Random index selection
    angle =100; %Vangle(idx_angle);              % angle (degree)
