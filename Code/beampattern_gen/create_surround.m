function [h_n,a_n,information] = create_surround(PlotRoom, example)

    %load handel.mat
    mFlag = 0;
    %disp("arrived to create")
    % Parameters
    ...
    % room dimensions 
    x_lim = 8; %+ 3*rand(1);         % x position (m) - number in the range [6,9](m)
    y_lim = 8; %+ 3*rand(1);         % y position (m) - number in the range [6,9](m)
    ...
    angleOrientation = 0; %Vangle(idx_angle); 
    ...
    mic_x = x_lim/2; %2.5 + (x_lim-5)*rand(1);   % [1.5, x_room - 1.5] 0.5m for the source and 1 for min radius
    mic_y = y_lim/2; %0.5 + (y_lim-3)*rand(1);   % [0.5, y_room - 1.5] 0.5m for the source    
    ...
    radius = (lim_radius-1.8) + 1.8;  % [1, lim_radius](m) - The radius from the center of the array
    Vangle = (0:1:180);                    % Possible values for angle
    %idx_angle = randi(numel(Vangle));       % Random index selection
    angle = Vangle(example+1);              % angle (degree)
 ...
 % =====Ilai Z only test:====
    ...
    %idx_angle = randi(numel(Vangle));             % Random index selection
    angle_n = 120; %Vangle(idx_angle);                  % angle (degree)
